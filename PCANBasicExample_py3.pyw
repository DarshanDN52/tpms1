...
##################################################################################### 
    # To enable Windows event support, install the pywin32 library:
    # pip install pywin32
    try:
        # Use importlib to perform a dynamic import so static analyzers won't fail
        import importlib
        win32event = importlib.import_module('win32event')
        WINDOWS_EVENT_SUPPORT = True
    except Exception:
        # If the module is not available, ensure the name exists and mark support as disabled
        win32event = None
